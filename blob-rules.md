...
Use cases
Vercel Blob is a great solution for storing blobs
 that need to be frequently read. Here are some examples suitable for Vercel Blob:
...
Use addRandomSuffix: true: This automatically adds a unique random suffix to your pathnames:
const blob = await put('avatar.jpg', imageFile, {
  access: 'public',
...
Upload a robots.txt file to the root of your blob store with appropriate directives
...
Upload a robots.txt file to your blob store as described above
Use the "Remove URLs" tool in Google Search Console to request removal
Choosing your Blob store region
You can create Blob stores in any of the 19 regions. Use the region selector in the dashboard at blob store creation time, or use the CLI with the --region option.
